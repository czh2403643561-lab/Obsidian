...
import type { CandidateShop, CandidateShopInput, CandidateShopSource, PercentilePreset, Shop, ShopFilters, ShopSortField, SortDirection } from "./types";
...
function SmallShopList({ hidden, candidateShops, onToggleCandidate }: { hidden: boolean; candidateShops: CandidateShop[]; onToggleCandidate: (shop: CandidateShopInput, source: CandidateShopSource) => void }) {
      ...
      {shops.length > 0 && <>
        ...
        <section className="list-card shop-list-card">
          ...
          <div className="table-wrap"><table className="shop-table" data-sort-field={sort.field ?? undefined}><colgroup><col className="shop-col-name" /><col className="shop-col-type" /><col className="shop-col-rating" /><col className="shop-col-recent-sales" /><col className="shop-col-total-sales" /><col className="shop-col-gmv" /><col className="shop-col-products" /><col className="shop-col-creators" /><col className="shop-col-videos" /><col className="shop-col-lives" /></colgroup><thead><tr><th className="shop-name-column">店铺</th><th>店铺类型</th><th className="shop-number-column"><ShopSortButton field="rating" sort={sort} onSort={handleSort} /></th><th className="shop-number-column"><ShopSortButton field="recentSales" sort={sort} onSort={handleSort} /></th><th className="shop-number-column"><ShopSortButton field="totalSales" sort={sort} onSort={handleSort} /></th><th className="shop-number-column"><ShopSortButton field="recentGmv" sort={sort} onSort={handleSort} /></th><th className="shop-number-column">带货 / 在店商品</th><th className="shop-number-column"><ShopSortButton field="creators" sort={sort} onSort={handleSort} /></th><th className="shop-number-column"><ShopSortButton field="videos" sort={sort} onSort={handleSort} /></th><th className="shop-number-column"><ShopSortButton field="lives" sort={sort} onSort={handleSort} /></th></tr></thead>
            <tbody>{filteredShops.map((shop) => { const saved = candidateShops.some((candidate) => candidate.key === candidateKey(shop.id, shop.url) && candidate.snapshots["小店列表"]); return <tr key={`${shop.id}-${shop.url}`}><td className="shop-name-column"><div className="shop-name-entry"><a className="shop-name-link" href={shop.url} target="_blank" rel="noreferrer"><span className="shop-avatar"><Store size={17} /></span><span><strong title={shop.name}>{shop.name}</strong><small>{shop.deliveryCategory || "未填写分类"}</small></span></a><button aria-label={saved ? "取消收藏" : "收藏到候选池"} className={`shop-candidate-button${saved ? " saved" : ""}`} onClick={() => onToggleCandidate(shop, "小店列表")} title={saved ? "取消收藏" : "收藏到候选池"}>{saved ? <BookmarkCheck size={19} /> : <Bookmark size={19} />}</button></div></td><td><div className="shop-cell"><span>{shop.shopType}</span><small>{shop.managed ? `全托管：${shop.managed}` : "全托管：—"}</small></div></td><td className="shop-number-column"><span className="rating-badge">{shop.rating ? shop.rating.toFixed(1) : "—"}</span></td><td className="shop-number-column"><span className="metric-value highlight">{formatCompact(shop.recentSales)}</span><small className="metric-label">近 7 天</small></td><td className="shop-number-column"><span className="metric-value">{formatCompact(shop.totalSales)}</span><small className="metric-label">累计销量</small></td><td className="shop-number-column"><span className="metric-value amount-value">£{formatCompact(shop.recentGmv)}</span><small className="metric-label">近 7 天</small></td><td className="shop-number-column"><span className="metric-value">{formatCompact(shop.promotedProductCount)}</span><small className="metric-label">在店 {formatCompact(shop.totalProducts)}</small></td><td className="shop-number-column"><span className="metric-value">{formatCount(shop.creators)}</span></td><td className="shop-number-column"><span className="metric-value">{formatCount(shop.videos)}</span></td><td className="shop-number-column"><span className="metric-value">{formatCount(shop.lives)}</span></td></tr>; })}</tbody></table>
...
export default function ShopBoard({ hidden = false, candidateShops, onToggleCandidate }: { hidden?: boolean; candidateShops: CandidateShop[]; onToggleCandidate: (shop: CandidateShopInput, source: CandidateShopSource) => void }) {
  ...
  return (
    <main id="shops-workspace" className="workspace" hidden={hidden} aria-hidden={hidden}>
      ...
      <SmallShopList hidden={activeView !== "shops"} candidateShops={candidateShops} onToggleCandidate={onToggleCandidate} />
      <SellerRankBoard kind="crossborder" hidden={activeView !== "crossborder"} candidateShops={candidateShops} onToggleCandidate={onToggleCandidate} />
      <SellerRankBoard kind="local" hidden={activeView !== "local"} candidateShops={candidateShops} onToggleCandidate={onToggleCandidate} />
    </main>
  );
}
