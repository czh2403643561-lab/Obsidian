...
import { Check, ChevronDown, ChevronLeft, ChevronRight, Copy, Download, ExternalLink, Filter, HelpCircle, MoreVertical, PencilLine, Search, SlidersHorizontal, X } from "lucide-react";
...
type DetailMetric = { key: string; group: "销量" | "流量"; label: string; value: number | null; format: "money" | "count" | "rate" };
...
const formatMetric = (metric: DetailMetric, currencySymbol: string): string => metric.format === "money" ? formatMoney(metric.value, currencySymbol) : metric.format === "rate" ? formatRate(metric.value) : formatCount(metric.value);
...
const detailMetricsFor = (product: BusinessProductRecord, trafficMode: "页面浏览量" | "去重访客数"): DetailMetric[] => {
  ...
  return [
    { key: "gmv", group: "销量", label: "商品卡 GMV", value: card.gmv, format: "money" },
    { key: "orders", group: "销量", label: "订单数", value: product.orders, format: "count" },
    { key: "skuOrders", group: "销量", label: "SKU 订单数", value: card.skuOrders, format: "count" },
    { key: "units", group: "销量", label: "商品成交件数", value: card.units, format: "count" },
    { key: "aov", group: "销量", label: "AOV", value: card.aov, format: "money" },
    { key: "customers", group: "销量", label: "预计客户数", value: card.customers, format: "count" },
    { key: "impressions", group: "流量", label: unique ? "去重商品曝光次数" : "商品曝光次数", value: unique ? card.uniqueImpressions : card.impressions, format: "count" },
    { key: "clicks", group: "流量", label: unique ? "去重点击次数" : "商品点击量", value: unique ? card.uniqueClicks : card.clicks, format: "count" },
    { key: "ctr", group: "流量", label: unique ? "去重点击率" : "商品点击率", value: unique ? card.uniqueCtr : card.ctr, format: "rate" },
    { key: "addToCarts", group: "流量", label: unique ? "加购用户数" : "加购次数", value: unique ? card.addToCartUsers : card.addToCarts, format: "count" },
    { key: "addToCartRate", group: "流量", label: unique ? "去重加购率" : "加购率", value: unique ? card.uniqueAddToCartRate : card.addToCartRate, format: "rate" },
    { key: "ctor", group: "流量", label: unique ? "去重CTOR" : "CTOR", value: unique ? card.uniqueCtor : card.ctor, format: "rate" },
  ];
};

function ProductDetail({ product, batch, onBack }: { product: BusinessProductRecord; batch: BusinessBatch; onBack: () => void }) {
  const [channel, setChannel] = useState("全部");
  const [selected, setSelected] = useState(["skuOrders", "impressions"]);
...
export default function ProductAnalyticsList({ batch }: { batch: BusinessBatch | null }) {
  ...
  if (selectedProduct && batch) return <ProductDetail product={selectedProduct} batch={batch} onBack={() => setSelectedProduct(null)} />;
