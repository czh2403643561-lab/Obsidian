...
import type { CandidateShop, CandidateShopInput, CandidateShopSource, PercentilePreset, SellerRank, SellerRankFilters, SellerRankSortField, SortDirection } from "./types";
...
interface SellerRankWorkspaceState {
  ...
  sort: { field: SellerRankSortField | null; direction: SortDirection | null };
  parseNotice: string;
...
const hasActiveFilters = (filters: SellerRankFilters): boolean => Boolean(
  filters.search || filters.salesAmountMin || filters.salesAmountMax || filters.salesMin || filters.salesMax ||
...
export default function SellerRankBoard({ kind, hidden, candidateShops, onToggleCandidate }: { kind: SellerBoardKind; hidden: boolean; candidateShops: CandidateShop[]; onToggleCandidate: (shop: CandidateShopInput, source: CandidateShopSource) => void }) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [workspace, setWorkspace] = usePersistedState<SellerRankWorkspaceState>(`seller-${kind}`, initialWorkspaceState);
  const { sellers, fileName, sort, parseNotice, foundHeaderCount } = workspace;
  ...
  useEffect(() => {
    if (!sellers.some((seller) => !Number.isInteger(seller.originalIndex))) return;
    setWorkspace((current) => ({
      ...current,
      sellers: current.sellers.map((seller, index) => Number.isInteger(seller.originalIndex) ? seller : { ...seller, originalIndex: index }),
    }));
  }, [sellers, setWorkspace]);
  ...
  const filteredSellers = useMemo(() => {
    ...
    return sellers.filter((seller) => {
      if (searchQuery && !seller.name.toLowerCase().includes(searchQuery)) return false;
      ...
      if (!sort.field || !sort.direction) return (a.originalIndex ?? sellers.indexOf(a)) - (b.originalIndex ?? sellers.indexOf(b));
      const difference = a[sort.field] - b[sort.field];
      if (difference !== 0) return sort.direction === "desc" ? -difference : difference;
      return (a.originalIndex ?? sellers.indexOf(a)) - (b.originalIndex ?? sellers.indexOf(b));
    });
  ...
  const handleSort = (field: SellerRankSortField) => setWorkspace((current) => {
  ...
  const loadFile = async (file?: File) => {
    ...
    try {
      ...
      setWorkspace({ sellers: result.sellers, fileName: file.name, importedAt: new Date().toISOString(), draftFilters: { ...initialFilters }, appliedFilters: { ...initialFilters }, sort: { field: null, direction: null }, parseNotice: notices.join("；"), foundHeaderCount: result.foundHeaders.length });
  ...
  return <section className="board-view" hidden={hidden} aria-hidden={hidden}>
    ...
    {sellers.length > 0 && <><section className="overview-grid"><div className="overview-main"><span>导入数量</span><strong>{formatCount(sellers.length)}</strong><small>家店铺</small></div><div className="overview-stat"><span>当前结果</span><strong>{formatCount(filteredSellers.length)}</strong><small>符合筛选条件</small></div><div className="overview-stat"><span>带货分类</span><strong>{categories.length}</strong><small>已识别分类</small></div><div className="overview-stat accent-stat"><span>当前排序</span><strong>{sort.field ? sortLabels[sort.field] : "原始榜单顺序"}</strong><small>{sort.direction === "desc" ? "从高到低" : sort.direction === "asc" ? "从低到高" : "未启用排序"}</small></div></section>
      {parseNotice && <div className="notice info-notice"><CircleHelp size={17} /><span>{parseNotice}</span></div>}
      <section className="filter-card"><div className="filter-card-header"><div className="filter-title"><Filter size={17} /><strong>筛选条件</strong><span>{hasActiveFilters(appliedFilters) ? "已启用组合筛选" : "全部店铺"}</span></div><div className="filter-card-actions">{hasDraftChanges && <span className="pending-filter-note">条件已修改，点击应用筛选生效</span>}<button className="apply-button" onClick={() => setWorkspace((current) => ({ ...current, appliedFilters: { ...current.draftFilters } }))} disabled={!hasDraftChanges}><Check size={15} /> 应用筛选</button>{hasAnyFilterValues && <button className="text-button" onClick={clearFilters}><RefreshCw size={14} /> 清除筛选</button>}</div></div><div className="filter-grid seller-filter-grid"><div className="filter-block search-filter"><label htmlFor={`${kind}-search`}>搜索店铺</label><div className="search-input"><Search size={15} /><input id={`${kind}-search`} type="search" value={draftFilters.search} onChange={(event) => updateFilter("search", event.target.value)} placeholder="输入店铺名称" /></div></div><RangeInput label="销售额 (£)" minValue={draftFilters.salesAmountMin} maxValue={draftFilters.salesAmountMax} onMinChange={(value) => updateFilter("salesAmountMin", value)} onMaxChange={(value) => updateFilter("salesAmountMax", value)} /><RangeInput label="销量" minValue={draftFilters.salesMin} maxValue={draftFilters.salesMax} onMinChange={(value) => updateFilter("salesMin", value)} onMaxChange={(value) => updateFilter("salesMax", value)} /><RangeInput label="带货商品数" minValue={draftFilters.productsMin} maxValue={draftFilters.productsMax} onMinChange={(value) => updateFilter("productsMin", value)} onMaxChange={(value) => updateFilter("productsMax", value)} /><div className="filter-block"><label htmlFor={`${kind}-category-filter`}>带货分类</label><div className="select-wrap"><select id={`${kind}-category-filter`} value={draftFilters.category} onChange={(event) => updateFilter("category", event.target.value)}><option value="all">不限</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select><ChevronDown size={15} /></div></div><div className="filter-block"><label htmlFor={`${kind}-creators-filter`}>达人 <span className="label-help" title="百分位基于当前导入的完整卖家榜数据计算"><CircleHelp size={13} /></span></label><div className="select-wrap"><select id={`${kind}-creators-filter`} value={draftFilters.creators} onChange={(event) => updateFilter("creators", event.target.value as PercentilePreset)}><option value="all">不限</option><option value="p15">最低 15% · ≤ {formatCompact(thresholds.creators.p15)}</option><option value="p20">最低 20% · ≤ {formatCompact(thresholds.creators.p20)}</option><option value="p50">最低 50% · ≤ {formatCompact(thresholds.creators.p50)}</option></select><ChevronDown size={15} /></div></div><div className="filter-block"><label htmlFor={`${kind}-videos-filter`}>视频数 <span className="label-help" title="百分位基于当前导入的完整卖家榜数据计算"><CircleHelp size={13} /></span></label><div className="select-wrap"><select id={`${kind}-videos-filter`} value={draftFilters.videos} onChange={(event) => updateFilter("videos", event.target.value as PercentilePreset)}><option value="all">不限</option><option value="p15">最低 15% · ≤ {formatCompact(thresholds.videos.p15)}</option><option value="p20">最低 20% · ≤ {formatCompact(thresholds.videos.p20)}</option><option value="p50">最低 50% · ≤ {formatCompact(thresholds.videos.p50)}</option></select><ChevronDown size={15} /></div></div></div><div className="filter-footnote"><Search size={13} /> 百分位档位根据本次导入的完整卖家榜数据动态计算，支持多个条件同时生效。</div></section>
      <section className="list-card shop-list-card"><div className="list-card-header shop-list-card-header"><div><div className="list-heading"><h2>{name}</h2><span className="result-pill">{formatCount(filteredSellers.length)} 个结果</span>{regionLabel && <span className="market-badge"><Globe2 size={17} /><span>{regionLabel}</span></span>}</div><p>点击店铺名称打开 EchoTik 原店铺页</p></div><div className="list-actions"><Store size={15} /> {presetName(appliedFilters.creators)} 达人 · {presetName(appliedFilters.videos)} 视频</div></div><div className="table-wrap"><table className="seller-table" data-sort-field={sort.field ?? undefined}><colgroup><col className="seller-col-name" /><col className="seller-col-category" /><col className="seller-col-sales-amount" /><col className="seller-col-sales" /><col className="seller-col-products" /><col className="seller-col-creators" /><col className="seller-col-videos" /><col className="seller-col-lives" /></colgroup><thead><tr><th className="seller-name-column">店铺</th><th>带货分类</th><th className="seller-number-column"><SellerSortButton field="salesAmount" sort={sort} onSort={handleSort} /></th><th className="seller-number-column"><SellerSortButton field="sales" sort={sort} onSort={handleSort} /></th><th className="seller-number-column"><SellerSortButton field="promotedProductCount" sort={sort} onSort={handleSort} /></th><th className="seller-number-column"><SellerSortButton field="creators" sort={sort} onSort={handleSort} /></th><th className="seller-number-column"><SellerSortButton field="videos" sort={sort} onSort={handleSort} /></th><th className="seller-number-column"><SellerSortButton field="lives" sort={sort} onSort={handleSort} /></th></tr></thead><tbody>{filteredSellers.map((seller) => { const source: CandidateShopSource = kind === "crossborder" ? "跨境卖家榜" : "本土卖家榜"; const saved = candidateShops.some((candidate) => candidate.key === candidateKey(seller.id, seller.url) && candidate.snapshots[source]); return <tr key={`${seller.id}-${seller.url}`}><td className="seller-name-column"><div className="shop-name-entry"><a className="shop-name-link" href={seller.url} target="_blank" rel="noreferrer"><span className="shop-avatar"><Store size={17} /></span><span><strong title={seller.name}>{seller.name}</strong><small>{seller.collectedAt || "—"}</small></span></a><button aria-label={saved ? "取消收藏" : "收藏到候选池"} className={`shop-candidate-button${saved ? " saved" : ""}`} onClick={() => onToggleCandidate(seller, source)} title={saved ? "取消收藏" : "收藏到候选池"}>{saved ? <BookmarkCheck size={19} /> : <Bookmark size={19} />}</button></div></td><td><span className="seller-category">{seller.deliveryCategory}</span></td><td className="seller-number-column"><span className="metric-value highlight">£{formatCompact(seller.salesAmount)}</span></td><td className="seller-number-column"><span className="metric-value">{formatCompact(seller.sales)}</span></td><td className="seller-number-column"><span className="metric-value">{formatCount(seller.promotedProductCount)}</span></td><td className="seller-number-column"><span className="metric-value">{formatCount(seller.creators)}</span></td><td className="seller-number-column"><span className="metric-value">{formatCount(seller.videos)}</span></td><td className="seller-number-column"><span className="metric-value">{formatCount(seller.lives)}</span></td></tr>; })}</tbody></table>{!filteredSellers.length && <div className="no-results"><div className="empty-icon small"><Search size={20} /></div><strong>没有符合条件的店铺</strong><span>尝试放宽筛选范围，或清除筛选重新查看。</span><button className="text-button" onClick={clearFilters}>清除筛选</button></div>}</div></section>
    </>}
