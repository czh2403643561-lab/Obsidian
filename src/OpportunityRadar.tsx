...
import type { OpportunityCategoryWorkspace, OpportunityFilters, OpportunityLevel, OpportunityRadarState, OpportunityRecord, OpportunitySnapshot, OpportunitySortField, OpportunityTab, OpportunityTag, OpportunityTrendStatus } from "./types";
...
const trendLabels: Record<OpportunityTrendStatus, string> = { growing: "连续增强", new: "新出现", crowded: "竞争涌入", cooling: "机会降温" };
...
const buildTrendRecords = (current: OpportunityRecord[], previous: OpportunityRecord[]): TrendRecord[] => {
  ...
  return matches.map(({ record, previous: previousRecord }) => {
    ...
    const status: OpportunityTrendStatus = crowded ? "crowded" : cooling ? "cooling" : searchDelta > 0 ? "growing" : "cooling";
